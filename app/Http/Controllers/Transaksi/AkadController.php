...
class AkadController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($customers->values())
                ...
                ->addColumn('iph', fn($row) =>
                    $row->persyaratan?->IPH
                        ? '<i class="fas fa-check-circle text-success"></i>'
                        : '<i class="fas fa-times-circle text-danger"></i>'
                )

                ->addColumn('shgb', fn($row) =>
                    $row->persyaratan?->SHGB
                        ? '<i class="fas fa-check-circle text-success"></i>'
                        : '<i class="fas fa-times-circle text-danger"></i>'
                )

                ->addColumn('pajak', fn($row) =>
                    $row->persyaratan?->pajak
                        ? '<i class="fas fa-check-circle text-success"></i>'
                        : '<i class="fas fa-times-circle text-danger"></i>'
                )
                ...
                ->rawColumns(['checkbox', 'lokasi_rumah', 'iph', 'shgb', 'pajak', 'action'])
                ->make(true);
        }
    ...
    public function cetakDetailPDF($id)
    {
        ...
        foreach ($detailList as $index => $item) {
            ...
            // Styling status
            $statusClass = ($status == 'hadir') ? 'status-hadir' : 'status-belum';
        ...
        $jumlahHadir      = $detailList->where('status', 'hadir')->count();
    ...
    public function cetakDetailExcel($id)
    {
        ...
        foreach ($detailList as $index => $item) {
            ...
            $sheet->setCellValue("D{$row}", ucwords($status));
