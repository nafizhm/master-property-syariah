                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form id="formData" enctype="multipart/form-data">
                                    ...
                                    <div class="row mb-3">
                                        <label for="jenis_proyek" class="col-sm-2 col-form-label">Jenis Proyek</label>
                                        <div class="col-sm-3">
                                            <select class="form-control select-jenis-proyek" name="jenis_proyek"
                                                id="jenis_proyek">
                                                <option value=""></option>
                                        ...
                                        <label for="id_proyek_bangunan"
                                            class="col-sm-2 col-form-label d-none label-bangunan">Proyek Bangunan</label>
                                        <div class="col-sm-3 d-none select-bangunan">
                                            <select class="form-control select-proyek-bangunan" name="id_proyek_bangunan"
                                                id="id_proyek_bangunan">
                                                <option value=""></option>
                                                @foreach ($proyek_bangunanList as $data)
                                                    <option value="{{ $data->id }}">{{ $data->nama_proyek }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <label for="id_jalan" class="col-sm-2 col-form-label d-none label-jalan">Proyek
                                            Jalan</label>
                                        <div class="col-sm-3 d-none select-jalan">
                                            <select class="form-control select-proyek-jalan" name="id_jalan" id="id_jalan">
                                                <option value=""></option>
                                                @foreach ($proyek_jalanList as $data)
                                                    <option value="{{ $data->id }}">{{ $data->nama_proyek }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <label for="id_saluran" class="col-sm-2 col-form-label d-none label-saluran">Proyek
                                            Saluran</label>
                                        <div class="col-sm-3 d-none select-saluran">
                                            <select class="form-control select-proyek-saluran" name="id_saluran"
                                                id="id_saluran">
                                                <option value=""></option>
                                                @foreach ($proyek_saluranList as $data)
                                                    <option value="{{ $data->id }}">{{ $data->nama_proyek }}</option>
                                                @endforeach
                                            </select>
                                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#jenis_proyek').on('change', function() {
                var val = $(this).val()
                $('.label-bangunan, .select-bangunan, .label-jalan, .select-jalan, .label-saluran, .select-saluran')
                    .addClass('d-none')
                if (val === 'Bangunan') {
                    $('.label-bangunan, .select-bangunan').removeClass('d-none')
                } else if (val === 'Jalan') {
                    $('.label-jalan, .select-jalan').removeClass('d-none')
                } else if (val === 'Saluran') {
                    $('.label-saluran, .select-saluran').removeClass('d-none')
                }
            })
        })

        $(document).ready(function() {
            $('.select-jenis-proyek').select2({
                theme: "bootstrap4",
                width: '100%',
                placeholder: "Pilih Jenis Proyek",
            });
            $('.select-proyek-bangunan').select2({
                theme: "bootstrap4",
                width: '100%',
                placeholder: "Pilih Proyek Bangunan",
            });
            $('.select-proyek-jalan').select2({
                theme: "bootstrap4",
                width: '100%',
                placeholder: "Pilih Proyek Jalan",
            });
            $('.select-proyek-saluran').select2({
                theme: "bootstrap4",
                width: '100%',
                placeholder: "Pilih Proyek Saluran",
            });
        ...
        document.getElementById('jenis_proyek').addEventListener('change', function() {
            ...
            if (this.value === '1') {
                document.querySelector('.label-bangunan').classList.remove('d-none');
                document.querySelector('.select-bangunan').classList.remove('d-none');
            } else if (this.value === '2') {
                document.querySelector('.label-jalan').classList.remove('d-none');
                document.querySelector('.select-jalan').classList.remove('d-none');
            } else if (this.value === '3') {
                document.querySelector('.label-saluran').classList.remove('d-none');
                document.querySelector('.select-saluran').classList.remove('d-none');
            }
        });
