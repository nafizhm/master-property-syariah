                        ...
                        <div class="card">
                            <div class="card-header p-3">
                                <div class="d-flex align-content-center justify-content-between">
                                    <h3 class="font-weight-bold text-lg">Data Proyek Bangunan</h3>
                                    <div class="d-flex align-items-center">
                                        @if ($permissions['tambah'])
                                            <button type="button" class="btn btn-sm btn-primary" data-toggle="modal"
                                                data-target="#modalForm">
                                                <i class="fas fa-plus"></i> Tambah Proyek Bangunan
                                            </button>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <form id="formData">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-3">
                                    <input type="text" id="jumlah_unit" name="jumlah_unit" class="form-control"
                                        readonly>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-3">
                                    <div class="input-group">
                                        <input type="text" name="tipe_rumah" id="tipe_rumah"
                            ...
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label">Volume Pekerjaan</label>
                                <div class="col-sm-3">
                                    <div class="input-group">
                                        <input type="text" name="volume_pekerjaan" id="volume_pekerjaan"
...
@push('scripts')
    <script>
        $(document).on('click', '[data-target="#modalForm"]', function() {
            $('#modalFormLabel').text('Tambah Proyek Bangunan');
        });
        ...
        function updateNilaiPekerjaan() {
            const volume = parseNumber($('#volume_pekerjaan').val());
            const harga = parseNumber($('#harga_satuan').val());
            const total = volume * harga;
            $('#nilai_pekerjaan').val(formatNumber(total));
        }

        function updateVolumePekerjaan() {
            const jumlahUnit = parseNumber($('#jumlah_unit').val());
            const tipeRumah = parseNumber($('#tipe_rumah').val());
            const volume = jumlahUnit * tipeRumah;
            $('#volume_pekerjaan').val(formatNumber(volume));
            updateNilaiPekerjaan();
        }

        $(document).on('input', '#volume_pekerjaan, #harga_satuan', function() {
        ...
        $(document).on('input', '#tipe_rumah', function() {
            let value = $(this).val().replace(/[^0-9]/g, '');
            value = formatNumber(value);
            $(this).val(value);
            updateVolumePekerjaan();
        ...
        $(function() {
            var table = $('.data-table').DataTable({
                ...
                ordering: false,
                responsive: true,
                ajax: "{{ route('proyek-bangunan.index') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
        ...
        $(document).on('click', '.edit-button', async function() {
            ...
            $.get(url, async function(response) {
                if (response.status === 'success') {
                    $('#modalFormLabel').text('Edit Proyek Bangunan');
                    ...
                    $('#blok').val(response.blok).trigger('change');

                    $('#tipe_rumah').val(response.data.tipe_rumah);
                    $('#volume_pekerjaan').val(response.data.volume_pekerjaan);
                    ...
                    let countChecked = 0;
                    $('#unit-rumah-wrapper input[type=checkbox]').each(function() {
                        const val = $(this).val();
                        const checked = response.unit.includes(val);
                        $(this).prop('checked', checked).prop('disabled', true);
                        if (checked) countChecked++;
                    });

                    $('#jumlah_unit').val(countChecked);
        ...
        $('#modalForm').on('hidden.bs.modal', function() {
            ...
            $('.select-blok').val('').trigger('change');
        ...
        $('#formData').on('submit', function(e) {
            ...
            let url = id ?
                '{{ route('proyek-bangunan.update', ['proyek_bangunan' => ':id']) }}'.replace(':id', id) :
                '{{ route('proyek-bangunan.store') }}';
